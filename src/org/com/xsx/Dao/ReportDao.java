...
public class ReportDao {
	...
	public static Object[] QueryReportList(QueryReportFilterData filterData){
		...
		tempstr = filterData.getSimpleid();
		if(tempstr != null){
		...
		//如果为合格，则显示所有合格的数据
		 //如果为不合格，则显示所有不合格的数据
		//如果为审核，则显示所有已审核数据
		tempstr = filterData.getReportresult();
		if(tempstr != null){
			
			//查询已审核
			if(tempstr == "已审核")
				hql.append(" AND RESULT IS NOT NULL");
		...
		hql1.append(hql);
		hql1.append(" limit " 
				+ filterData.getPageindex()*filterData.getPagesize()
				+ "," 
				+filterData.getPagesize());
